...
Public Class SettlementForm
    ...
    Private Sub LoadData()
        Try
            ...
            Dim sql As String = $"SELECT id, title, remarks, creationtime FROM xipunum_erp_type WHERE type='结算方式' AND superior='0' {searchFilter} ORDER BY id DESC"
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        ' 权限检查
        If Not HasOperationPermission("33") Then
            ShowWarning("没有结算方式管理权限！")
            Return
        End If

        Try
            ' 检查是否存在
            Dim checkSql As String = $"SELECT * FROM xipunum_erp_type WHERE type='结算方式' AND title='{SafeSQL(txtName.Text)}' AND superior='0'"
            ...
            If checkDt.Rows.Count > 0 Then
                ShowWarning("该结算方式已存在！")
                Return
            End If

            Dim sql As String
            If String.IsNullOrEmpty(currentId) Then
                sql = $"INSERT INTO xipunum_erp_type (type, superior, title, remarks, cjuser, creationtime) VALUES ('结算方式', '0', '{SafeSQL(txtName.Text)}', '{SafeSQL(txtRemarks.Text)}', '{SafeSQL(UserAccount)}', '{GetOperationDate()}')"
            Else
                sql = $"UPDATE xipunum_erp_type SET title='{SafeSQL(txtName.Text)}', remarks='{SafeSQL(txtRemarks.Text)}', updatetime='{GetOperationDate()}' WHERE id='{currentId}'"
            End If
            DatabaseModule.ExecuteCommand(sql)
            AddSystemLog(If(String.IsNullOrEmpty(currentId), "添加", "编辑"), If(String.IsNullOrEmpty(currentId), "添加结算方式", "编辑结算方式"), "结算方式：" & txtName.Text)
            ShowSuccess("保存成功！")
            LoadData()
    ...
    Private Sub btnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
        ...
        Try
            DatabaseModule.ExecuteCommand($"DELETE FROM xipunum_erp_type WHERE id='{currentId}'")
            AddSystemLog("删除", "删除结算方式", "结算方式：" & txtName.Text)
            ShowSuccess("删除成功！")
            LoadData()
            btnReset_Click(Nothing, Nothing)
        Catch ex As Exception
